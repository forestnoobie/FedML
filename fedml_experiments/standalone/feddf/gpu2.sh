...
do
	for EPOCH in 5 10
	do
		python ./main_feddf.py --gpu 1 --client_num_in_total 20 --client_num_per_round ${CLIENT_NUM} \
		...
		--valid_ratio 0.1 --unlabeled_data_dir /home/osilab7/hdd/cifar \
		...
		--coninit_load --condense_reinit_model --lambda_fedprox 0.01
		...
		--model resnet8_no_bn --partition_method hetero --partition_alpha ${ALPHA} --comm_round 100 \
		 --epochs  ${EPOCH} --lr 0.1 --client_optimizer sgd --frequency_of_the_test 1 --ci 1 \
		--valid_ratio 0.1 --server_steps 0 \
		--lambda_fedprox 0.01 --pname prox
	done
done